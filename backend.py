...
from automode_logic import (
    AutomodeRequest,
    start_automode_logic,
    automode_messages,
    automode_progress,
)
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=ALLOWED_ORIGINS.split(";") or ["*"],
...
def create_jwt(data: object, expiry: int):
    # Ensure that no 'set' is in the payload; convert to list if necessary
    if isinstance(data, set):
        data = list(data)

    expiration_time = datetime.utcnow() + timedelta(minutes=expiry)
...
async def google_auth(request: AuthCode):
    token = request.token
    ...
    access_token = create_jwt(data, 60)
    refresh_token = create_jwt(data, 336)

    # Remove the 'client_id' key if it exists
    data.pop("client_id", None)
    data.pop("domain", None)

    # Return user information or perform further processing
...
async def refresh_access_token(request: RefreshToken):
    ...
    # Step 2: Create a new access token using the valid claims from the refresh token
    new_access_token = create_jwt(data=payload, expiry=60)
...
@app.post("/automode")
async def start_automode(request: Request, token: str = Depends(verify_access_token)):
    automode_request = AutomodeRequest(**request.json())
    sync_project_state_with_fs()  # Ensure state is synced before starting
    return StreamingResponse(
        start_automode_logic(automode_request), media_type="text/event-stream"
    )


@app.get("/automode")
async def start_automode_get(message: str, token: str = Depends(verify_access_token)):
...
async def get_automode_status(token: str = Depends(verify_access_token)):
...
def safe_path_operation(func: Callable):
    ...
    async def wrapper(*args, **kwargs):
        ...
        print(
            f"[DEBUG] safe_path_operation: original_path={path}, full_path={full_path}"
        )
...
async def create_project(
    request: ProjectRequest, path: str, token: str = Depends(verify_access_token)
):
    try:
        ...
        if request.template == "react":
            ...
            await create_file(
                os.path.join(project_path, "index.js"),
                'console.log("Hello, Node.js!");',
            )
        elif request.template == "python":
            await create_file(
                os.path.join(project_path, "main.py"), 'print("Hello, Python!")'
            )
        ...
        return {
            "message": f"{request.template} project created successfully at {relative_path}"
        }
...
async def create_folder_endpoint(
    path: str = Query(...), token: str = Depends(verify_access_token)
):
    try:
...
async def upload_file(
    file: UploadFile = File(...), token: str = Depends(verify_access_token)
):
    try:
...
@app.post("/search")
async def search(query: SearchQuery, token: str = Depends(verify_access_token)):
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error performing search: {error_details}"
        )


@app.post("/clear_state")
async def clear_project_state(token: str = Depends(verify_access_token)):
    try:
...
# Chat endpoint
@app.post("/chat")
async def chat(request: ChatRequest, token: str = Depends(verify_access_token)):
...
async def download_projects(token: str = Depends(verify_access_token)):
...
async def console_execute_command(
    request: CommandRequest, token: str = Depends(verify_access_token)
):
    global current_working_directory
    try:
        ...
        if cmd == "cd":
            ...
            return await handle_touch(
                command_parts[1] if len(command_parts) > 1 else "",
                current_working_directory,
            )
        else:
            output = await execute_shell_command(
                request.command, current_working_directory
            )
...
async def handle_cd(path):
    ...
    try:
        ...
        if new_path.is_dir():
            ...
            return {
                "result": f"Changed directory to: {await get_relative_cwd()}",
                "cwd": await get_relative_cwd(),
            }
        else:
            return {
                "result": f"Directory not found or access denied: {path}",
                "cwd": await get_relative_cwd(),
            }
    except Exception as e:
        return {
...
async def handle_mkdir(dirname, cwd):
    try:
        ...
        return {
            "result": f"Directory created: {dirname}",
            "cwd": await get_relative_cwd(),
        }
    except Exception as e:
        return {
...
async def handle_touch(filename, cwd):
    try:
        ...
        return {
            "result": f"Error touching file: {str(e)}",
            "cwd": await get_relative_cwd(),
        }
...
async def refresh_project_state_endpoint(token: str = Depends(verify_access_token)):
    try:
