...
const ClaudeApp = () => {
  ...
  const handleImageUpload = async (
    ...
    if (file) {
      ...
      try {
        const response = await axiosInstance.post(`/analyze_image`, formData, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });
        setMessages((prev) => [
          ...prev,
    ...
    if (input.trim()) {
      try {
        ...
        const eventSource = new EventSource(
          ...
          }/automode?message=${encodeURIComponent(input)}`
        );
        ...
        eventSource.onerror = (error) => {
          ...
          setMessages((prev) => [
            ...prev,
            { role: 'system', content: 'Error: Automode failed' },
          ]);
          eventSource.close();
          setIsAutoMode(false);
        };
  ...
  const handleSearch = async () => {
    if (searchQuery.trim()) {
      ...
      try {
        ...
        setMessages((prev) => [
          ...prev,
          { role: 'system', content: errorMessage, isHtml: false },
        ]);
      }
      setSearchQuery('');
    }
  ...
  const listFiles = async (path: string) => {
    try {
      const response = await axiosInstance.get(`/list_files`, {
        params: { path },
      });
      ...
      if (axios.isAxiosError(error) && error.response) {
        ...
        toast({
          ...
          description:
            'There was a problem loading the file list. Please try again.',
          status: 'error',
          duration: 5000,
          isClosable: true,
        });
  ...
  const createFolder = async () => {
    ...
    if (folderName) {
      try {
        await axiosInstance.post(`/create_folder`, null, {
          params: { path: `${currentDirectory}/${folderName}` },
        });
        listFiles(currentDirectory);
      } catch (error) {
  ...
  const createFile = async () => {
    ...
    if (fileName) {
      try {
        await axiosInstance.post(`/create_file`, null, {
          params: { path: `${currentDirectory}/${fileName}` },
        });
        listFiles(currentDirectory);
      } catch (error) {
  ...
  const readFile = async (fileName: string) => {
    try {
      ...
      const response = await axiosInstance.get(`/read_file`, {
  ...
  const deleteFile = async () => {
    ...
    try {
      await axiosInstance.delete(`/delete_file`, {
  ...
  const clearMessages = async () => {
    try {
      setMessages([]);
      await axiosInstance.post(`/clear_state`);
      toast({
  ...
  const viewSetupInstructions = async (fileName: string) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: 'assistant', content: `Claude: ${response.data.response}` },
      ]);
      ...
      setMessages((prev) => [
        ...
        {
          role: 'system',
          content: 'Error: Failed to get setup instructions. Please try again.',
        },
      ]);
    } finally {
      setIsLoading(false);
  ...
  const handleDownloadProjects = async () => {
    try {
      const response = await axiosInstance.get(`/api/download_projects`, {
        responseType: 'blob',
      ...
      toast({
        ...
        description:
          'There was an error downloading the projects folder. Please try again.',
        status: 'error',
        duration: 5000,
        isClosable: true,
      });
                ...
                <VStack spacing={4} align='stretch' width='100%'>
                  ...
                  <HStack justifyContent='space-between'>
                    ...
                    <Button
                      ...
                      colorScheme={isAutoMode ? 'red' : 'yellow'}
                    >
                  ...
                  <HStack justifyContent='space-between'>
                    <Menu>
                      ...
                      <MenuList>
                        ...
                        <MenuItem onClick={() => createProjectTemplate('node')}>
                          Node.js App
                        </MenuItem>
                        <MenuItem
                          onClick={() => createProjectTemplate('python')}
                        >
                          Python App
                        </MenuItem>
                      </MenuList>
                ...
                <VStack spacing={4} align='stretch'>
                  <HStack justifyContent='space-between'>
                    ...
                    <HStack>
                      ...
                      <Button
                        onClick={() => listFiles(currentDirectory)}
                        colorScheme='yellow'
                      >
                        Refresh
                      </Button>
